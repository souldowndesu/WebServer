...
临时预览使用现有服务器的多工作区租约机制，只占用 `agent-1` 已登记的独立 loopback 端口，不创建外部云服务，也不开放公网端口。使用结束后应停止前台预览，并执行 `python3 tools/workspace_runtime.py release --session <active-session>` 归还工作区租约与测试端口。要长期多人使用，必须另走环境变更，安装 main 派生的只读服务并配置 HTTPS。
