...
工作区预览使用现有服务器的多工作区租约机制，只占用 `agent-1` 已登记的独立 loopback 端口，不创建外部云服务，也不开放公网端口。使用结束后应停止前台预览，并执行 `python3 tools/workspace_runtime.py release --session <active-session>` 归还工作区租约与测试端口。单部署区版本必须另走环境变更，从已审核的 `main` 安装只读代码和模板；没有 HTTPS 时仍只允许通过 SSH 隧道访问。
